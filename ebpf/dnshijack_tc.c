// SPDX-License-Identifier: GPL-2.0
//
// dnshijack_tc.c — eBPF XDP program
//
// Intercepts DNS queries (UDP/53).  For each (qname, qtype) pair found in
// the DNS_CACHE map, builds a DNS response in-place and redirects the packet
// back to the sender without forwarding to unbound/bind.
//
// Build:
//   clang -O2 -g -Wall -target bpf \
//         -D__TARGET_ARCH_x86 \
//         -I/usr/include/bpf \
//         -c dnshijack_tc.c -o dnshijack_tc.o
//
// The userspace loader embeds dnshijack_tc.o and uses aya to load/attach it.

#include <linux/bpf.h>
#include <linux/if_link.h>
#include <linux/if_ether.h>
#include <linux/ip.h>
#include <linux/in.h>
#include <linux/udp.h>
#include <bpf/bpf_helpers.h>
#include <bpf/bpf_endian.h>

// ── Constants ────────────────────────────────────────────────────────────────

#define DNS_PORT        53
#define DNS_PAYLOAD_MAX 768
#define QNAME_MAX_LEN   256

// ── Map types (must match dnshijack-common/src/lib.rs) ───────────────────────

struct dns_key {
    __u8  qname[QNAME_MAX_LEN];  // wire-format, lowercased, zero-padded
    __u16 qtype;
    __u8  _pad[6];
} __attribute__((packed, aligned(8)));

struct dns_value {
    __u8  payload[DNS_PAYLOAD_MAX];  // pre-encoded DNS response (after txid)
    __u16 payload_len;
    __u8  _pad[6];
} __attribute__((packed, aligned(8)));

// ── BPF map ──────────────────────────────────────────────────────────────────
// Use legacy map metadata so userspace can load the object without BTF.
struct bpf_map_def {
    __u32 type;
    __u32 key_size;
    __u32 value_size;
    __u32 max_entries;
    __u32 map_flags;
};

struct bpf_map_def SEC("maps") DNS_CACHE = {
    .type = BPF_MAP_TYPE_HASH,
    .key_size = sizeof(struct dns_key),
    .value_size = sizeof(struct dns_value),
    .max_entries = 1024 * 1024,
    .map_flags = BPF_F_NO_PREALLOC,
};

// ── Helper: in-place IP checksum update ──────────────────────────────────────
// Incremental update for a 16-bit field change (RFC 1624).
static __always_inline __u16
csum_update(__u16 old_csum, __u16 old_val, __u16 new_val)
{
    __u32 csum = (~old_csum & 0xffff)
               + (~old_val  & 0xffff)
               + new_val;
    csum = (csum >> 16) + (csum & 0xffff);
    csum += (csum >> 16);
    return ~csum;
}

// ── XDP program ──────────────────────────────────────────────────────────────

SEC("xdp")
int dnshijack_xdp(struct xdp_md *ctx)
{
    void *data     = (void *)(long)ctx->data;
    void *data_end = (void *)(long)ctx->data_end;
    __u32 old_pkt_len = (__u32)((__u8 *)data_end - (__u8 *)data);

    // ── Ethernet ─────────────────────────────────────────────────────────────
    struct ethhdr *eth = data;
    if ((void *)(eth + 1) > data_end)
        return XDP_PASS;
    if (eth->h_proto != bpf_htons(ETH_P_IP))
        return XDP_PASS;

    // ── IPv4 (standard 20-byte header) ───────────────────────────────────────
    struct iphdr *ip = (void *)(eth + 1);
    if ((void *)(ip + 1) > data_end)
        return XDP_PASS;
    if (ip->ihl != 5)           // no IP options
        return XDP_PASS;
    if (ip->protocol != IPPROTO_UDP)
        return XDP_PASS;

    // ── UDP ──────────────────────────────────────────────────────────────────
    struct udphdr *udp = (void *)(ip + 1);
    if ((void *)(udp + 1) > data_end)
        return XDP_PASS;
    if (udp->dest != bpf_htons(DNS_PORT))
        return XDP_PASS;

    // ── DNS header ───────────────────────────────────────────────────────────
    __u8 *dns = (void *)(udp + 1);
    if (dns + 12 > (__u8 *)data_end)
        return XDP_PASS;

    __u16 udp_len = bpf_ntohs(udp->len);
    if (udp_len < sizeof(struct udphdr) + 12)
        return XDP_PASS;

    __u16 dns_payload_len = udp_len - sizeof(struct udphdr);

    // Byte 2-3: flags.  QR bit (bit 15) must be 0 (query).
    __u16 dns_flags = ((__u16)dns[2] << 8) | dns[3];
    if (dns_flags & 0x8000)
        return XDP_PASS;   // already a response

    // Byte 4-5: QDCOUNT must be 1
    __u16 qdcount = ((__u16)dns[4] << 8) | dns[5];
    if (qdcount != 1)
        return XDP_PASS;

    // ── Parse qname ──────────────────────────────────────────────────────────
    struct dns_key key = {};
    __u16 dns_rem = dns_payload_len - 12;
    int   key_pos = 0;
    int   name_done = 0;
    int label_rem = 0;
    __u8 *p = dns + 12;         // start of question section

    // Parse qname from linear XDP packet bytes.
    for (int i = 0; i < QNAME_MAX_LEN; i++) {
        if (dns_rem == 0)
            return XDP_PASS;
        if (p + 1 > (__u8 *)data_end)
            return XDP_PASS;

        __u8 b = *p++;
        dns_rem--;

        if (label_rem == 0) {
            if (b > 63)             // compression pointer — unexpected in query
                return XDP_PASS;
            if (key_pos < QNAME_MAX_LEN)
                key.qname[key_pos++] = b;
            if (b == 0) { name_done = 1; break; }
            label_rem = b;
        } else {
            __u8 lc = (b >= 'A' && b <= 'Z') ? b + 32 : b;
            if (key_pos < QNAME_MAX_LEN)
                key.qname[key_pos++] = lc;
            label_rem--;
        }
    }

    if (!name_done)
        return XDP_PASS;

    // p now points to qtype (2 bytes) + qclass (2 bytes)
    if (dns_rem < 4)
        return XDP_PASS;
    if (p + 4 > (__u8 *)data_end)
        return XDP_PASS;
    __u16 qtype = ((__u16)p[0] << 8) | p[1];
    key.qtype = qtype;

    // ── Map lookup ───────────────────────────────────────────────────────────
    struct dns_value *val = bpf_map_lookup_elem(&DNS_CACHE, &key);
    if (!val)
        return XDP_PASS;

    __u32 payload_len = val->payload_len;
    if (payload_len == 0 || payload_len > DNS_PAYLOAD_MAX)
        return XDP_PASS;

    // ── Craft response ───────────────────────────────────────────────────────
    // Transaction ID (first 2 DNS bytes)
    __u16 txid = ((__u16)dns[0] << 8) | dns[1];

    // Sizes
    __u32 new_dns_len  = 2 + payload_len;         // txid + payload
    __u32 new_udp_len  = sizeof(struct udphdr) + new_dns_len;
    __u32 new_ip_total = sizeof(struct iphdr)  + new_udp_len;
    __u32 new_pkt_len  = sizeof(struct ethhdr) + new_ip_total;

    // Resize packet (extend or truncate)
    int delta = (int)new_pkt_len - (int)old_pkt_len;
    if (bpf_xdp_adjust_tail(ctx, delta) != 0)
        return XDP_PASS;

    // Reload pointers after resize/adjust
    data     = (void *)(long)ctx->data;
    data_end = (void *)(long)ctx->data_end;
    eth = data;
    ip  = (void *)(eth + 1);
    udp = (void *)(ip  + 1);
    dns = (void *)(udp + 1);

    // Re-validate all header boundaries after adjust so the verifier can
    // prove subsequent direct packet accesses are in-bounds.
    if ((void *)(eth + 1) > data_end)
        return XDP_PASS;
    if ((void *)(ip + 1) > data_end)
        return XDP_PASS;
    if ((void *)(udp + 1) > data_end)
        return XDP_PASS;

    if ((void *)dns + new_dns_len > data_end)
        return XDP_PASS;

    // ── Swap Ethernet MACs ───────────────────────────────────────────────────
    __u8 tmp_mac[6];
    __builtin_memcpy(tmp_mac, eth->h_dest, sizeof(tmp_mac));
    __builtin_memcpy(eth->h_dest, eth->h_source, sizeof(tmp_mac));
    __builtin_memcpy(eth->h_source, tmp_mac, sizeof(tmp_mac));

    // ── Swap IP src/dst; update total length; fix checksum ───────────────────
    __be32 tmp_ip = ip->saddr;
    ip->saddr = ip->daddr;
    ip->daddr = tmp_ip;

    __u16 old_tot_len = bpf_ntohs(ip->tot_len);
    __u16 new_tot_len_h = (__u16)new_ip_total;
    __u16 old_csum = bpf_ntohs(ip->check);
    ip->check    = bpf_htons(csum_update(old_csum, old_tot_len, new_tot_len_h));
    ip->tot_len  = bpf_htons(new_tot_len_h);

    // ── Swap UDP ports; fix length; zero checksum (valid for UDP/IPv4) ───────
    __be16 tmp_port = udp->source;
    udp->source = udp->dest;
    udp->dest   = tmp_port;
    udp->len    = bpf_htons((__u16)new_udp_len);
    udp->check  = 0;  // RFC 768: 0 means "not computed"

    // ── Write DNS response ───────────────────────────────────────────────────
    // Older verifiers need a constant bound proof for the first two DNS bytes.
    if ((void *)(dns + 2) > data_end)
        return XDP_PASS;

    dns[0] = (__u8)(txid >> 8);
    dns[1] = (__u8)(txid & 0xff);

    __u8 *dst = dns + 2;
    if ((void *)(dst + payload_len) > data_end)
        return XDP_PASS;

    // Copy pre-computed payload (after txid).
    // Older verifiers are sensitive to variable map-value indexing. Keep map
    // accesses at compile-time-constant offsets by handling each 32-byte block
    // with constant indices only.
    #pragma unroll
    for (int c = 0; c < (DNS_PAYLOAD_MAX / 32); c++) {
        const __u32 off = (__u32)c * 32;
        if (payload_len <= off)
            continue;

        if (payload_len >= off + 32) {
            __u8 chunk_buf[32];
            __builtin_memcpy(chunk_buf, val->payload + off, sizeof(chunk_buf));
            if ((void *)(dst + off + sizeof(chunk_buf)) > data_end)
                return XDP_PASS;
            __builtin_memcpy(dst + off, chunk_buf, sizeof(chunk_buf));
            continue;
        }

        // Tail bytes in the final partial block: write one byte at a time.
        #pragma unroll
        for (int t = 0; t < 32; t++) {
            const __u32 idx = off + (__u32)t;
            if (payload_len <= idx)
                break;

            if ((void *)(dst + idx + 1) > data_end)
                return XDP_PASS;
            __u8 b = val->payload[idx];
            dst[idx] = b;
        }
    }

    // ── Send the forged response back out the ingress NIC ───────────────────
    return XDP_TX;
}

char LICENSE[] SEC("license") = "GPL";
